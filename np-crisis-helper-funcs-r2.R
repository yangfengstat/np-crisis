# ROUND-2 VARIANT of np-crisis-helper-funcs.R: identical except that the
# reserves_match.mse.alpha() model branch is generalized from i.model == 2
# to i.model >= 2, so AdaBoost (i.model = 5) can drive the Section 5.2
# robustness recalibration.
...
# Remove columns and rows with all NAs
      ...
      } else if (i.model >= 2){  # ROUND-2: generalized so any nproc method (e.g., ada) can drive the calibration
